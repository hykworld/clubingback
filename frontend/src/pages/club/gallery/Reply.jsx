...
const Reply = ({ postType, postId }) => {
  ...
  const {
    ...
    queryFn: async () => {
      const response = await axiosInstance.get(`http://3.133.122.248:4000/replies/${postId}`);
  ...
  const mutation = useMutation({
    mutationFn: (newComment) => {
      return axiosInstance.post(`http://3.133.122.248:4000/replies/add/${newComment.postId}`, newComment);
    },
  });
  ...
  const deleteMutation = useMutation({
    mutationFn: ({ replyId, writer }) => {
      return axiosInstance.delete(`http://3.133.122.248:4000/replies/delete/${replyId}`, {
  ...
  const editMutation = useMutation({
    mutationFn: ({ replyId, writer, comment }) => {
      return axiosInstance.put(`http://3.133.122.248:4000/replies/edit/${replyId}`, {
        writer,
        comment,
      });
    },
