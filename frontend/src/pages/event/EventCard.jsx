...
const EventCard = ({ eventData, onClose }) => {
  ...
  const uploadImageToServer = async (imageBlob) => {
    if (imageBlob) {
      ...
      try {
        const response = await axiosInstance.post('http://3.133.122.248:4000/events/upload', formData);
  ...
  const mutationRegister = useMutation({
    mutationFn: async (dataToSend) => {
      const response = await axiosInstance.post('http://3.133.122.248:4000/events/newEvent', dataToSend);
  ...
  const mutationUpdate = useMutation({
    mutationFn: async (dataToSend) => {
      const response = await axiosInstance.put(`http://3.133.122.248:4000/events/${dataToSend.eventId}`, dataToSend);
      return response.data;
    },
    onSuccess: (data) => {
